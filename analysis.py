...
class Analysis:
    def __init__(self):
        self.__ollama_llm = ChatOllama(model="llava", temperature=0.7)
    ...
    def seo_analysis(self, video_url: str, meta_data: dict, language: str='English'):
        ...
        format_instructions = self.seo_response_schema().get_format_instructions()
        ...
        template2 = PromptTemplate(
            template="""
                ...
                1. Exactly 35 trending tags/hashtags related to video content, ranked by potential traffic and relevance. For {platform}, optimize the tags according to platform best practices.
                ...
                3. Exactly {num_of_timestamps} timestamps with descriptive labels evenly distributed throughout the video (duration: {duration} seconds)
                4. 5-7 alternative titles suggesations ranked by SEO potential each under 60 characters for youtube appropriate length for {platform}

                {format_instructions}

                All content should be in {language} language.
            """, input_variables=['platform', 'title', 'analysis', 'num_of_timestamps', 'duration', 'language'], partial_variables={'format_instructions': format_instructions}
        )

        recommendation_chain = template2 | self.__ollama_llm
        response = recommendation_chain.invoke({'platform': platform, 'title': title, 'analysis': analysis, 'num_of_timestamps': num_of_timestamps, 'duration': duration, 'language': language})
